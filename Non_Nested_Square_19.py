...
neumann_fun1, info_conv = gmres(V1, b1, tol=10E-8)#, callback=make_callback())
...
An1 = np.dot(Pas.T, Atemp)
An1 = aslinearoperator(An1)

bn1 = np.dot(b0, Pas)

neumann_funn1, info_conv = gmres(An1, bn1, tol=10E-8)#, callback=make_callback())

#un1 = gf(sp1, coefficients=neumann_funn1)
un1 = gf(sp0, coefficients=np.dot(neumann_fun1, Pm))
